  ...
  const deleteJob = async () => {
    try {
      ...
      const res = await axios.delete(
        `${backendUri}/api/v1/job/deleteJob/${selectedJob?._id}`,{withCredentials:true}
      );
      if (res.data.success) {
  ...
  const submitHandler = async () => {
    ...
    try {
      const res = await axios.post(
        `${backendUri}/api/v1/job/updateJob/${selectedJob?._id}`,
        input,
        ...
        <div className="bg-white mb-20 shadow-2xl max-w-5xl flex flex-col gap-4 rounded-lg p-4 mt-10 ">
          ...
          <div className="flex gap-7 justify-end">
            <Button
              onClick={deleteJob}
              variant="outline"
              className="bg-red-300 text-red-700 hover:bg-red-400 hover:text-red-800"
            >
              Delete Job
            </Button>
            <Button
